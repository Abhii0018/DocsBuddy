...
export default function UploadReports({ onFiles }){
  const inputRef = useRef(null)
  const [isDrag, setIsDrag] = useState(false)

  function handleClick(){ inputRef.current?.click() }
  function handleFiles(e){ const files = Array.from(e.target.files || []); onFiles?.(files) }
  function handleDrop(e){ e.preventDefault(); setIsDrag(false); const files = Array.from(e.dataTransfer.files || []); onFiles?.(files) }
