...
export default function AppointmentCard({ id, subject = 'General Consultation', date = 'TBD', timeWindow = '', status = 'pending', bookedOn = '', onDelete }) {
      ...
      >
        <div className="flex items-start justify-between mb-3">
          <div className="flex-1">
            <h3 className="text-lg font-semibold text-gray-800 mb-1">Subject: {subject}</h3>
          </div>
          <div className="flex items-center gap-2">
            <div className={`px-3 py-1 rounded-full text-xs font-semibold border ${statusClass} capitalize`}>
              {status}
            </div>
            {isPending && (
              <button
